...
- **EventBridge**: 毎日 UTC 9:00 に Lambda を起動
...
EventBridge のテストルール `aws-billing-discord-test` を ENABLED にすると 1 分間隔で実行されます。確認後は DISABLED に戻してください。
...
| `NotificationTime` | `cron(0 9 * * ? *)` | 通知スケジュール (UTC) |
